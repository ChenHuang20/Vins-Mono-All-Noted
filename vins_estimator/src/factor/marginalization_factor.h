...
class MarginalizationInfo
{
  public:
    ...
    void addResidualBlockInfo(ResidualBlockInfo *residual_block_info);//添加残差块(因子)相关信息（优化变量，待边缘化变量）
    void preMarginalize();
    void marginalize();
    ...
    std::unordered_map<long, int> parameter_block_size; //<优化变量内存地址,localSize> localSize是每个变量的维度
    int sum_block_size;
