...
{
    TicToc tic_toc;
    //parameters[0]是第一个参数，即para_Pose[imu_i], 7维，第imu_i帧变量
    Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);
    Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);

    //parameters[1]是第二个参数，即para_Pose[imu_j], 7维，第imu_j帧变量
    Eigen::Vector3d Pj(parameters[1][0], parameters[1][1], parameters[1][2]);
    Eigen::Quaterniond Qj(parameters[1][6], parameters[1][3], parameters[1][4], parameters[1][5]);

    //parameters[2]是第三个参数，即para_Ex_Pose[0], 7维，外参
    Eigen::Vector3d tic(parameters[2][0], parameters[2][1], parameters[2][2]);
    Eigen::Quaterniond qic(parameters[2][6], parameters[2][3], parameters[2][4], parameters[2][5]);

    //parameters[3]是第四个参数，即para_Feature[feature_index], 1维，逆深度λ
    //pts_i 是i时刻归一化相机坐标系下的3D坐标
    //第i帧相机坐标系下的的逆深度
    double inv_dep_i = parameters[3][0];
    //第i帧相机坐标系下的3D坐标，投影式右边坐标
