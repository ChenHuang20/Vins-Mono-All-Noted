...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    {
        // FeaturePerFrame：特征点[x,y,z,u,v,vx,vy]和td：IMU与cam同步时间差
    ...
    // 2. 若追踪次数小于20或者滑动窗口内帧的数目小于2，是关键帧
...
{
    for (auto &it_per_id : feature)//遍历所有三维点
    {
        it_per_id.used_num = it_per_id.feature_per_frame.size();//这个三维点一共落在的帧数
        if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))//这个三维点落在的帧数大于2（确保有两帧以上），且它的起始帧不是最后两帧（确保有两帧以上）
            continue;

        if (it_per_id.estimated_depth > 0)//这个三维点三角化过了，跳过
            continue;
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)//遍历这个三维点的所有帧特征（这里三角化用的是多帧的计算，而不是两帧）
        {
