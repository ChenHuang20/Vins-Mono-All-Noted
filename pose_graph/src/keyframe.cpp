                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    Matrix3d R_inital;
    Vector3d P_inital;
	//这里的world是vio，即漂移后的world,w2
	// T_w2_cj = T_w2_bj * T_b_c
	...
	//opencv 的pnp函数得到的位姿是rvec = R_c_w, t = t_c_w
	//回环帧ci的2d点和当前帧cj的3d点做PNP得T_ci_w2
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	*/
	...
	{
		//RANSAC PnP检测去除误匹配的点
		// pnp得到的是闭环帧在关键帧世界w2下的位姿T_w2_bi，再通过T_bi_w2 * T_w2_bj得到相对位姿T_bi_bj
		status.clear();
	    PnPRANSAC(matched_2d_old_norm, matched_3d, status, PnP_T_old, PnP_R_old);//PnP_T_old是T_w2_bi
	    reduceVector(matched_2d_cur, status);
	    reduceVector(matched_2d_old, status);
	    reduceVector(matched_2d_cur_norm, status);
	    reduceVector(matched_2d_old_norm, status);
