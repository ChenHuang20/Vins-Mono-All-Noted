...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    if (sequence_cnt != cur_kf->sequence)
    {
        ...
        sequence_loop.push_back(0);
        w_t_vio = Eigen::Vector3d(0, 0, 0);
        w_r_vio = Eigen::Matrix3d::Identity();//T_w1_w2
        m_drift.lock();
        t_drift = Eigen::Vector3d(0, 0, 0);
        r_drift = Eigen::Matrix3d::Identity();
        m_drift.unlock();
    ...
    cur_kf->getVioPose(vio_P_cur, vio_R_cur);//得到T_w2_j
    //T_w1_j = T_w1_w2 * T_w2_j
    vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
    vio_R_cur = w_r_vio * vio_R_cur;
    cur_kf->updateVioPose(vio_P_cur, vio_R_cur);//得到T_w1_j
	...
	if (loop_index != -1)
	{
        ...
        if (cur_kf->findConnection(old_kf))//计算当前帧与闭环帧的相对位姿
        {
            ...
            //重新计算当前帧位姿w_P_cur、w_R_cur,这一步相当于给当前帧执行“回环检测”
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    // db.add调用一次，数据库里的图片帧id(Entries)就加1，可以通过cout << "database info: " << db << endl;查看
    // 第四个参数a表示当前帧只和数据库里面Entries<=a的图片比较,这里frame_index - 50表示当前帧只和在自己50帧前的数据库里的关键帧比较
    ...
    //确保与相邻帧具有好的相似度评分
    //ret结果是降序排列的，ret[0].Score代表当前帧和数据库里最相似的那一帧的相似度
...
*/
    //找到回环候选帧，且关键帧ID大于50才考虑进行回环
    if (find_loop && frame_index > 50)
    {
        int min_index = -1;
        //找到最小帧号的匹配帧，最小帧号在ret里不一定评分最高
        for (unsigned int i = 0; i < ret.size(); i++)
        {
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        //取出最新一个待优化帧作为当前帧，当某帧检测到回环时，会将该帧的索引放到优化队列中。
        m_optimize_buf.lock();
        ...
        if (cur_index != -1)
        {
            ...
            m_keyframelist.lock();
            //max_length表明最大的优化变量个数，需要优化的关键帧为从第一次回环帧到当前帧间的所有关键帧
            ...
            // AngleLocalParameterization类主要的作用是指定yaw角优化变量是如何进行迭代更新的
            // 因为是4自由度优化，旋转部分仅仅需要优化yaw方向，而优化yaw方向需要把旋转矩阵变成欧拉角,还定义了自动求导
            ...
            int i = 0;
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)//遍历找到闭环的所有关键帧
            {
                //找到第一个闭环帧
                ...
                //add edge
                //添加sequentials edges, 顺序边指的是每个关键帧与它前面的几个关键帧所构建的边，这里最多与前面4帧构成边
                //比如i是7，则i与6,5,4,3帧分别有边
                //顺序变的测量值T_bi_bj来自估计的位姿T_w2_bj
                for (int j = 1; j < 5; j++)
                {
                    //sequence_array[i] == sequence_array[i-j]是保证关键帧和边连着的这些帧都在同一个图像序列内
                ...
                //add loop edge
                //如果当前帧存在回环
                //回环边的测量方程跟顺序边的测量方程是相同的，不同在于回环边的测量值T_bi_bj来自回环检测中pnp求解的结果。
                if((*it)->has_loop)
                {
            ...
            */
            ...
            m_drift.lock();
            //r_drift和t_drift是位姿图优化后的T_w1_w2，之前shift_r和shift_t是优化前的T_w1_w2
